...
class IdmapDomainService(CRUDService):
    ...
    async def idmap_from_smbconf(self):
        ...
        entries = {}
        next_id = 6
        workgroup = global_params['raw'].get('workgroup')
        security = global_params['raw'].get('security')

        for k, v in global_params['idmap'].items():
            if "*" in k:
                domain = DSType.DS_TYPE_DEFAULT_DOMAIN.name
            else:
                domain = k.strip("idmap config ")
                domain = domain.split(":")[0].strip()
            entry = k.split(":", 1)[1]
            self.logger.debug("k: %s, v %s, entry: %s", k, v, entry)
            if not entries.get(domain):
                entries[domain] = {}

            entries[domain].update({entry.strip(): v.strip()})

        for k, v in entries.items():
            self.logger.debug("k: %s, v: %s\n", k, v)

            try:
                range = v.pop('range').split("-")
            except KeyError:
                self.logger.debug("No range specified for domain [%s]\n", k)
                range = [-1, -1]
    ...
    async def idmap_to_smbconf(self, data=None):
        ...
        server_role = await self.middleware.call('smb.getparm', 'server_role', 'global')
        ...
        for i in idmap:
            if i['name'] == DSType.DS_TYPE_DEFAULT_DOMAIN.name:
                if ad_idmap and ad_idmap['backend'] == 'AUTORID':
                    continue
                domain = "*"
            elif i['name'] == DSType.DS_TYPE_ACTIVEDIRECTORY.name:
                if not ad_enabled:
                    continue
                if i['backend'] == 'AUTORID':
                    domain = "*"
                else:
                    domain = workgroup
    ...
    @private
    async def synchronize_idmap(self):
        ha_mode = await self.middleware.call("smb.get_smb_ha_mode")
        if ha_mode == "CLUSTERED":
            return
        ...
        to_check = (await self.middleware.call('smb.get_globals'))['idmap']
    ...
    async def ctdb_idmap_setup(self):
        ...
        efficient in ctdb.
        """
        existing_idmap = (await self.middleware.call('smb.get_globals'))['idmap']
